...
/**
 * The vEvent class encapsulates a single vEvent object by maintaining a map of
 * property names to one or more property values associated with that name.
 * A name is the unique property identifier such as SUMMARY.
 */
class vEvent
{
	/**
	 * An associative array where each key is the property name and each value
	 * is a vEventProperty array of properties which share that property name.
	 */
	var $_map;
	...
	/**
	 * Parses a vEvent from one or more lines. Lines that are not property
	...
	//   of the vEvent that belong to that group as well. Should be enabled if a
	...
	function deleteProperty($name,$delgroup=true,$from=0,$numelem=0)
	{
		...
		if(array_key_exists($name,$this->_map)) {
			...
			if($delgroup) {
				...
				if(count($delgroups)) {
					// not very efficient but should be ok for typically sized vEvents...
	...
	function setProperty($name, $value, $idx=0, $pvidx=0, $group='',$pvdelim=';')
	{
		...
		$idx = count($this->_map[$name]);
		$this->_map[$name][] = new vEventProperty();
		// init name and group
...
/**
 * The vEventProperty class encapsulates a single vEvent property consisting
...
class vEventProperty
{
	...
	/**
	 * Parses a vEvent property from one or more lines. Lines that are not
	...
	{
		static $types = array (
				'SUMMARY', 'DTSTAMP', 'DTSTART', 'DTEND', 'UID', 'LOCATION', 'DESCRIPTION', );
		static $values = array (
				'INLINE', 'URL', 'CID');
